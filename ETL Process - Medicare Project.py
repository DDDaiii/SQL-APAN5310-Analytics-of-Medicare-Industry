...
providers_misc = df[['nppes_entity_code','average_age_of_beneficiaries','average_age_of_beneficiaries']]
...
providers_misc_id_list = [providers_misc.providers_misc_id[providers_misc.entity_code == i].values[0] for i in df.nppes_entity_code]

# Add name_id to the main dataframe
df.insert(1, 'providers_misc_id', providers_misc_id_list)
...
# Add name_id to the main dataframe
df.insert(1, 'provider_types_id', provider_types_id_list)
...
# Map providers_misc_id
medicare_participants_id_list = [medicare_participants.medicare_participant_id[medicare_participants.med_participation == i].values[0] for i in df.medicare_participation_indicator]
...
providers = df[['npi','name_id','provider_types_id','address_id','medicare_participant_id','providers_misc_id']]


# Populate the database
providers[['npi','name_id','provider_types_id','address_id','medicare_participant_id','providers_misc_id']].     drop_duplicates().to_sql(name='providers', con=engine, if_exists='append', index=False)
...
# Map providers_misc_id
medicare_charges_id_list = [medicare_charges.charges_id[medicare_charges.submitted_charges_amount == i].values[0] for i in df.total_submitted_charges]
